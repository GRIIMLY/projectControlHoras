...
import co.com.samtel.ControlAccesos.util.ControlOrdenados;
...
public class viewControlAccesoController {
	...
	private static ControlOrdenados controlOrdenado = new ControlOrdenados();
	...
	public void getAllControlRegistrosOrd(int mes, int year, int diaI, int diaF) {
		List<ReportControlAccesoOrd> controlregistrosOrds = caoc.getControlAccesoOrdService().findAllControlRegistrosOrd(mes,
				year, diaI, diaF);
		controlOrdenado.reporteControlAccesosOrd(controlregistrosOrds);
		System.out.println("total de registros:" + controlregistrosOrds.size());
