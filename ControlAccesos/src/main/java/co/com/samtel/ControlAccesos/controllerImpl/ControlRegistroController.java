		...
		for (String fechasDia : fechas) {
			...
			for (Integer codUser : codUsers) {
				...
				for (ControlAccesosOrd controlAccesoO : controlAccesosOrd) {
					System.out.println(controlAccesoO.getId().getId());
					cont ++;
					...
					if (cont == 2) {		
						System.out.println("no salida " + salida);
						System.out.println("no entrada " + entrada);
						System.out.println(getControlAccesoOrdService().findDifBetweenHours(entrada,salida) + " codigo : " +  controlAccesoO.getTblcodigoUsuario().getCodigo());
						System.out.println("id del control de acceso ordenado : " + controlAccesoO.getId());
						if (controlAccesoO.getId().getId() == 2154) {
					...
					if (band5 == 1 && band6 == 1){
						System.out.println("no laborado " + noLaborado);
						System.out.println("no salida " + salida);
						System.out.println("no entrada " + entrada);
				...
				System.out.println(controlDiario.getId());	
//				ControlRegistro controlRegistro = new ControlRegistro(controlDiario.getId(),numeroRegistros, noLaborado, permanencia, controlDiario);
//				getControlRegistroService().save(controlRegistro);
				
				System.out.println("antes de registrar en la base de datos");
				System.out.println("permanencia " + permanencia);
				System.out.println("no laborado " +  noLaborado);
				getControlRegistroService().saveControlRegistro(numeroRegistros, noLaborado, controlDiario.getId(), permanencia);
				band5  =  0;
				band6 = 0;
				cont = 0;
			}
